...
image: feature.jpeg
description: This part zooms in on the difficulties of quantifying and measuring uncertainty by giving a succinct definition and taxonomy of uncertainty. I ask where uncertainty comes from, how we quantify it, what we do when we cant, and how the sources of uncertainty might impact the visualisation. I provide an example of how to define uncertainty with an example of forecast uncertainty.
...
If you ask me where, in the interconnected web of definitions uncertainty is introduced, I would say right at the start, when we define the sample space. If your sample space for whatever system or variable you are considering has more than one outcome, then we have some level of uncertainty. I would also argue that the sample space does have to come from a probability space, but the probability space itself does not need to be defined for the sample space to have uncertainty. Now this is not a mathematical definition of uncertainty (there doesn't seem to be one and making one is beyond my skillset) but it is a connection of uncertainty to the strict probability definitions set out in mathematics. The other things in this drawing, such as a clearly defined probability space, random variables, and state spaces help us quantify and describe uncertainty, but I don't think they are uncertainty in of themselves.

### An Intensional Definition
If we can't have a mathematical definition, we should still endeavor to give a intensional definition. Instead of directly connecting our definition to the mathematical framework that already exists around probability and statistics, we can just try and describe what it **is**. Unfortunately there is a lot of literature on this already, and most of it is not exactly what we are looking for.

Papers on uncertainty that do offer a definition often specify aspects of, categories of, or philosophical perspective on uncertainty. These specifications lead to definitions that are so narrow they are not particularly useful. That is to say, a definition that only covers either Baysian or Frequentist statistics is not broad enough for our uses. On the other hand the layperson definition of uncertainty is too broad. Caimbridge dictionary defines uncertainty as "a situation in which something is not known, or something that is not known or certain" [2]. It is difficult to turn this definition into a mathematical one. Although most papers don't agree on their definition of uncertainty, that doesn't mean there aren't any good ones. I am going to follow the definition set by by Walker and Rotmans (2003) in their paper "Defining Uncertainty"[3].
...
While "Defining Uncertainty" does not provide a strict mathematical definition, it does provide a taxonomy of uncertainty, which is in some ways, more useful. Lets start this section off with a simple plot hopefully everyone has seen before. This is a plot taken from Forecasting: Principles and Practice [@robsbook]. It is a simple depiction time series data, but I have also included a forecast (using an ARIMA model) that has some uncertainty surrounding it.
