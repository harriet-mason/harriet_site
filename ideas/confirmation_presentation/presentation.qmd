...
::: {.notes}
Lets work through a fictitious scenario so you can all have some experience working with and without an uncertainty visualisation. After all, the only thing better than seeing my point is literally experiencing it.
It is currently 9pm and you are sitting at home watching TV. Your tram is predicted to arrive at your stop in 9 minutes, and the time to walk to the tram is negligible. The tram will leave as soon as it arrives at its stop. For every minute you spend at home watching TV you gain 2 utility points, for every minute you spend at the tram stop waiting, you lose 1 utility point. If you miss your tram and have to wait for the next one you will automatically get a total of -10 utility points. The plot shows 4 leaving time options, which choice will maximise utility.
...
## Question 2: Tram Times #1  {.smaller}
...
#simulated
set.seed(1)
arrival_times <- as.POSIXct("2023-06-12 9:9:00") + 60*rnorm(100000, sd=3)
start <- as.POSIXct("2023-06-12 9:00:00")
leave_times <- seq(as.POSIXct("2023-06-12 8:59:30"),
                   as.POSIXct("2023-06-12 9:14:30"),
                   by = 60)
...
line_cols <- RColorBrewer::brewer.pal(6, "Set1")
...
tram_times %>%
  ...
  geom_segment(x=leave_times[6], xend=leave_times[6],
               colour=line_cols[6], y=-1, yend=0.3, size=1) +
  geom_label(x=leave_times[6], y=0.3, label= "F",
             fill = line_cols[6], colour="black") +
...
::: {.notes}
I can go back to the previous slide if you want, but the utility points should not be considered that deeply, they mostly exist to highlight that there is a correct answer. I recommend you just take a guess and try to answer this question by thinking about when you would actually arrive at the tram stop, it makes the problem easier.
:::
...
tram_times %>%
  ...
  geom_label(x=current_time, y=0.6,
             label="9:00pm", fill = "grey", colour="white") +
  geom_label(x=leave_times[4], y=0.31, label= "D",
             fill = line_cols[4], colour="white") +
  geom_segment(x=leave_times[4], xend=leave_times[4],
               colour=line_cols[4], y=-1, yend=0.3, size=1) +
  scale_x_datetime(breaks = my_breaks,
...
:::
::: {.notes}
Now, we are going to do the same thing again, except this time, I'm going to let you see the uncertainty distribution.
:::
...
#simulated
set.seed(1)
arrival_times <- as.POSIXct("2023-06-12 9:02:00") + 60*rexp(1000, rate=1/5)
start <- as.POSIXct("2023-06-12 9:00:00")
leave_times <- seq(as.POSIXct("2023-06-12 8:59:30"),
                   as.POSIXct("2023-06-12 9:14:30"),
                   by = 60)
...
max_arrival <- as.POSIXct("2023-06-12 9:20:00")
current_time <- as.POSIXct("2023-06-12 9:00:00")
arrival_times <- as.POSIXct("2023-06-12 9:02:00") + 60*rexp(50, rate=1/5)
tram_times <- tibble(arrival_times = arrival_times)
estimated <- mean(arrival_times)
# Make plot
tram_times %>%
...
```
::: {.notes}
I'm also going to inform you that I chose a different distribution for this slide so don't try and use the results from the previous slide.
:::
