...
subtitle: "Problems With The Current Landscape of Uncertainty Visualisation Experiments"
...
- uncertainty is used interchangably to express a visualisation of a distribution and a visualisation of an estimate that allows us to take that estimate "with a grain of salt".
- The way we test visualisations is completely antithetical to the way they are used
- MOST VISUALISATION PAPERS ONLY ASK ONE QUESTION. If we care about being able to get a large amount of information from a plot and some trade off in information, why don't we test that trade off??? 
- The plotting landscape is starting to look similar to that of movies. As high quality technology and the internet has become more widespread, it is becoming easier and easier for people without a huge media company backing them to make a movie. A similar thing can be seen in plotting. As niche and facinating data becomes more widespread and plotting software becomes more accessible, it becomes easier to make plots. As it becomes easier to make plots the plots people make become more unique and are less relevant to the research put out by academia.
- There is a infinite number of plots we can 
- to capture this people testing visualisations need to ask a questions that form small parts of the "gist" of a visualisation
- Obviously the best case scenario is to just visualise the parameter of interest, but at that point we should just use a table. A visualisation that would be better replaced by a table is functionally pointless.
- To get around this many visualisation papers ask questions that are related to the visualisation but not explicitly depicted in it
- an animated plot, an interactive plot and depictions of multiple plots are all just different ways to depict multiple plots with different expressions of exchangability
- Sometimes the visualisation 
- Point of a visualisation is to identify sweeping and complicated structures quickly.
- Often the information provided in a visualisation is purposely distinct from the question asked by ep
- some cases where the question asked has no real correct answer from the participants point of view but an accuracy metric is still used. @Hofmann2012 showed study participants a graph of two different marginal distributions and asked participants "In which plot is the blue group furthest to the right?". When I read this paper I was not sure if they meant  
Importantly, none of these cases reflect the way visualisations are made in practice. On what *planet* are visualisation authors purposely hiding the information they want to 
...
@fig-spatialuncert depicts four proposed methods of visualising spatial uncertainty that can be made with the `Vizumap` R package [@lucchesi2021vizumap]. These four plots each present uncertainty with varying degrees of success and failure. Plot (a) depicts a bivariate map which uses a bivariate colour palette which is created by blending two single hue colour palettes. One colour represents the variable of interest while the other represents uncertainty. There are two immediate problems with this method. First of all, uncertainty is being mapped with hue and saturation. @fig-huesatcol illustrates the differences between hue, saturation, and value when looking at a colour palette. @Maceachren2012 found hue and saturation to be two of the worst aesthetics to map to uncertainty as they don't have an intuitive connection to uncertainty and so participants were worse. Value does have a natural connection to uncertainty (lighter values equate to higher uncertainty and darker values equate to more certainty) so it is a much more appropriate choice. While the `Vizumap` data does depict areas of light and darkenss, they are largely irrlevant to the uncertainty measure causing our heuristics to lead us to the incorrect conclusions. The Value-Suppressing Uncertainty Palettes (VSUP) shown in @fig-vsup maps estimates to the hue and uncertainty to the value thereby creating a more intuitive plot [@Correll2018 ]. Additionally, at high levels of uncertainty VSUP only has one output colour, which explicitly prevents any decoding of one output colour without enforcing a binary encoding of significance [@Correll2018]. Unfortunately VSUP are not easy to combine with packages like `Vizumap` which leaves it still somewhat difficult to express this encoding in practice. Plot (b) from @fig-spatialuncert depicts a pixel map. Pixel maps are similar to HOPs plots since they present a sample of possible values for the estimate, rather than a single value and an uncertainty visualsiation. Some depictions of a pixel map are quite intuitive, such as those shown in @Bauer2015, however these cases link "uncertainty" to sparseness rather than a wide array of potential outcomes, however the effectivness of a pixel map is yet to be shown in any experients. The pixel map depicted in (b) might be more effective if values were written explicitly on top of the pixels because mapping a distribution of colours to numerical values is currently required to extract actual values from the plot which is a relatively difficult mental task. The pixel map is also quite computationally expensive and hard to interpret when the relevant geographical areas are small relative to a large map, so it is likely best used on simpler smaller geographical areas. Plot (c) uses orientation to express uncertainty which @Maceachren2012 found to have very poor results in expressing uncertainty and should be avoided. Additionally, if you want to bring an estimate into dispute it is harder to ignore if the uncertainty to the same feature as the estimate, which this plot does not do. Plot (d) is essentially just depicting a single value except instead of an estimate it expresses the probability of a threshold being met for each area. This plot was developed specifically for decision making relative to a single value in mind, and can be considered a visualisation of a single parameter @Kuhnert2018.

```{r}
#| echo: false
#| warning: false
#| message: false
#| label: fig-spatialuncert
#| fig-subcap: 
#|   - "Bivariate map"
#|   - "Pixel map"
#|   - "Glyph map"
#|   - "Exceedance probability map"
#| fig-cap: "The four visualisations of spatial uncertainty that can be made using the `Vizumap` package. These are the four visualisations that are provided as an example in the package vingette."
#| layout-ncol: 1
...
It is not only important to display the key features of our distribution, but we also need to display them in a way that best conveys the information. @Hofmann2012 showed that just changing the co-ordinates of a plot can result in change in the power of an line up test

Not only should the relevant distribution features be visualised according to priority, but it is also a good idea to drop the irrelevant features of a distribution. Spatial-temporal visualisation is always an area of research in visualisation largely because we seem to find it difficult to drop exchangability in those cases. @Nathonours found that a scatter plot is better than a line plot if you want to convey the correlation between two time series. Unfortunately we cannot be sure if this was influenced by swapping the conditional distribution for the joint distribution on the high priority position axis, or by dropping the visualisation of the time series exchangability feature. @fig-timescatter depicts 4 different version of the same data, each with a different combination of distribution depicted and whether or not we visualised data's exchangability feature. The example both highlights the importance of identifying what features are import and identifying what aspect of the plot we want to map it to. A line graph depicts the joint distribution in the same way a HOPs or slope graph does, by displying the two variables moving together rather than explicitly displaying their relationship in the position axis, because of this additional distribution features, such as the exchangability, might contribute to the visualsiation of the joint distribution by making the ordering more clear. This also illustrates that the distribuiton features aren't entirely independent and the cost of dropping the notion of exchangability changes depending on the visualised distribution. It is likely the combination of irelevant feature removal and highlighting the correct distribution that makes correlation easier to identify in a scatter plot, but it is not possible to conclude one or the other for certain.
...
#| fig-cap: "Four figures to illustrate the impact of changing the priority of the distribution displayed and presence of exchangability. Time is depicted with opacity in plots (c) and (d) to maintain the same information. More recent dates are darker. Could you order these four plots by how well they convey correlation?"
#| layout-ncol: 2
...
The relationship between plots and the distributions they present are not entirely alien to the current literature of visualisaiton, it is just a topic that is touched on sporadically. @Wickham2011 discusses how in different displays of product plots result in depictions of different marginal, conditional, and joint distributions. The lineup protocol expands the idea of considering the distribution that we are visualising and expands it to considering a visualisation to be a single outcome of a larger distribution and ask visualisation authors to consider the distribution that might have generated a plot [@Buja2009; @Wickham2010]. This method can be used to identify false findings and perform complicated statistical tests [@Chowdhury; @Hofmann2012].


@Hofmann2012 compared a visualisation of mass, outcomes, and parameters (using a boxplot) and asked

- [@kosslyn2006graph] Assumption that charts are most effective when they show enough information to solve the task at hand, but not too much to hamper the observer
