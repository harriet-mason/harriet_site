...
The importance of using the correct visualisation for the correct question is best seen with an example. The study done by [@Hullman2015] is a great illustration in the importance of understanding your goal when visualising uncertainty. The study asked participants to provide some numerical properties of a distribution using a HOPs, error bar or violin plot. When shown a single distribution participants were asked about the mean of the distribution, the probability of an outcome being above some threshold (indicated on the plot with a red dot), or the probability of an outcome being between two given values. When shown two distributions the participants were asked "How often is measurement of solute B larger than the measurement of solute A?", and when shown three distributions "How often is measurement of solute B larger than the measurement of solute A and solute C?". @fig-examples shows an example error bar plot and violin plot for the two distribution case. The study decided which technique was better using absolute error between the subjects response and the true value. They specified that the error bars are not confidence intervals as they represent the true underlying distribution, not the sampling distribution of the mean. They found that the HOPS plots outperformed the violin and error bar plots in the two and three distribution cases, but performed worse in the univeriate case when there was high variance or when being used to answer questions about the mean.
...
# Slope Graph Data
slope_data <- scatter_data %>%
  mutate(pairs = 1:100) %>%
  pivot_longer(soluteA:soluteB, 
               names_to = "solute", 
               values_to = "ppm") %>%
  mutate(solute= ifelse(solute=="soluteA","Solute A", "Solute B"))

# Slopegraph plot
slope_data %>%
  ggplot(aes(x=solute, y=ppm, group=pairs, colour=biggerb)) +
  geom_point() +
  geom_line() +
  labs(y="Parts Per Million (PPM)")+
  scale_y_continuous(breaks=ticks, limits=c(0,100)) +
...
bar_data <- scatter_data %>%
  mutate(biggersolute = ifelse(biggerb=="Yes", "Solution B", "Solution A")) %>%
  select(-biggerb) %>%
  mutate(dummy = "d")

# Normal bar chart
bar_data %>%
  ggplot(aes(x=biggersolute, fill=biggersolute)) +
  geom_bar() +
  labs(y = "Proportion (%)",
       title = "% of draws that will have more of that solute") +
  scale_y_continuous(breaks=ticks, limits=c(0,100)) +
  theme_classic() +
  theme(axis.title.x=element_blank(), 
        axis.text.x=element_text(size=15),
...
The error bar plot and the violin plot in @fig-examples puts its primary focus on the distribution of each solution. These plots visualize the marginal distributions of solution A and B and implies we should be unconcerned with the relationship between the solutions and especially unconcerned with specific questions about it, such as, what is $P(B>A)$. The scatter plot in @fig-alternatives draws attention to the the relationship between the values of Solution A and Solution B, and even indicates what we should be looking for with a secondary feature (colour). This plot still maintain some information about the distribution (you can tell that we seem to have to have two independent normal distributions) however it is no longer of primary importance, the plot is now telling us that the general relationship between the solutions is the most important feature. The stacked bar chart in @fig-alternatives sets $P(B>A)$ as the primary feature of interest but drops entirely any notion of the distribution or features of the relationship. What should become apparent comparing these plots is that there is a constant trade off. There are only so many aesthetics we can map information to, and those aesthetics have a hierarchy of how easy they are the process. What is implicit every time we make a plot

This is why the conversation and research around error bars is very frustrating to read. Lets take a look at the accuracy rank of an error bar.

Plotting is a balancing act where we are  up the value of the useful context (in this case, the distribution and actual values of the PPM) and the information required

holding onto the context that we think is important (such as the marginal) and the relevant information bringing the relevant information to the forefront of the viewers attention. This trade off is often ignored when we visualise uncertainty, and papers (such as the Hullman paper) try to find a "better" plot rather than asking what information we are trying to uncover and what information we are willing to to bring that information to the forefront. What is frustrating is that this simple fact is not only well known within the data visualisation community, but Hullman herself acknowledges it in her paper.


The two plots that visualised the distribution depict the two concentrations as two independent random variables with little concern to their relationship. The two relationship plots highlighted that what we care about is actually care about is the relationship, and the colour highlighted the question (is A bigger than B) as something of secondary interest. These depitions still maintain some information about the distribution (you can tell that we seem to have to have two independent normal distributions) however it is no longer of primary importance. The stacked bar chart highlighted that the proportion is the aspect of the data we care about, while the normal bar chart highlighted that the difference in the two proportions is of primary importance. Any depiction of the distribution has been removed. 

It may seem obvious that the bar charts make the question easier to answer, and you may want to say "Well we want a plot that can answer a large number of questions, not just a single question about frequency". My response to this is.. why? Why do we need a plot that can "see" everything? Plots are similar to people in the sense that expecting one person to have a complete and flawless view of something as complicated as society ignores that one of the things that has allowed humanity to flourish is that there are so many of us. It allows each person to specialise and be good at something important to them rather than trying to be mediocre at everything. Similarly, no plot can see every possible thing you might want to know about your data. Leaning into and uncovering the strengths of each plot is the best way to research them because it reflects the ways in which we actually incorperate plotting into our diagram. While general principals are helpful, fundamentally each plot for each question should be as unique as the person who made it.

The plots in @fig-examples and @fig-alternatives also highlight that the idea of there being such a thing a "plots for distributions" and "plots for relationships" and such are a bit nonsense. The violin plot and the scatter plot were the only two that illustrated the data was a normal distribution but the error bar plot is technically a plot of a "distribution". I think this issue is largely why error bar plots get a lot of flack as being a bad plot, I think they are just being misused. 
...
to which I respond, you didn't test for that either. Identifying which plots can answer a large number of questions and which questions they can answer is the experiement I suggest in project 2. What is important to acknowledge about this question is that the QUESTION itself was about the PROPRORTION of an event. How often was B greater than A. While it was ABOUT a random distribution (although in statistics realistically everything is) because we wanted to know about a proprotion, the visualisation should reflect that. Assuming people can somehow infer the hidden information in a plot and then designing experients around is so flawed in logic I cannot understand it.

Additionally, a HOPs plot isn't even technically a visualisation of a distribution in some sense, it is a visualisation of a simulated dataset from a distribution. Identifying what aspect of the statistical framework you want to visualise and WHY is important in understanding how to build your plot. Since the pre
...
When deciding a plots purpose, it is useful to ask yourself what is being plotted to each aspect of the accuracy hierarchy.

- people couldn't answer that question with a DIFFERENT DISTRIBUTION PLOT
- plotting mapped to hierarchy
...
ignore questions about significance (the exact scenario that) assuming that answering a question about the probability (What is P(B>A)?) is a precursor to  


instead of asking a large range of irrelevant question The error bar plot shows which 

Second of all, the paper acknowledges issues with statistical understanding but misinterprets them as issues with the plot. Every plot we display has

First of all, the study ignores the questions that would be ideal for an error bar or violin plot.


- placebo rule, studys compare to placebo is bad, should compare to next best alternative
...
### Looking the method behind data vis papers
- there is a habit of comparing plots to NOTHING (error bar significance test) or to an alternative that is NOT "next best"
Third of all, there is no reason to have specifically compared a HOPs to a
