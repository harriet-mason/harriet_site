...
This is not an exhaustive list of every possible feature of a distribution, but rather the four most commonly visualised aspects of a distribution. Additionally, these features are not entirely distinct, particular visualsiations of parameters or samples may also double as a depiction of mass. Therefore, a graphic that is designed to depict one feature of a distribution may depict multiple features of multiple distributions. It also compares multiple ways of showing distribution to see which has more power.There are also uncertainty visualisation taxonomies that cross the boundaries of this feature based taxonomy, for example @Grewal2021 categorised uncertainty visualisations based on discreteness of the distribution (a sub aspect of mass) and the domain expertise (which was similar to the number/complexity of the features depicted). @Hofmann2012 classified uncertainty distributions by the level of data summarisation (a measure adjacent to the number of data points used). Despite these alternatives I suggest this taxonomy because it allows us to properly evaluate the results of any visualisation paper (not just uncertainty specific ones). This framework makes it easier to untangle whether it is the information presented or the graphic itself that creates a difference in our ability to draw information from a visualisation. 

## Part 3: Hierarchy

### Example: Looking to spatial uncertainty
Often our goal in visualisation is not to perform a statistical test or to make a decision based on a single estimate, but to identify some underlying structure in our data or make a decision based on a large number of connected systems. When there are many features of the data that are competing for our interest, we often don't have as much flexibility in how we choose to visualise uncertainty. Complicated features of our distributions, such as mass, are thrown to the wayside to make way for for complicated features of the data such as spatial dependence. Additionally, if we want to visualise the sampling distribution of a large number of estimates that have a lot of dependent variables, even visualising a simple feature of our distribution becomes difficult. 
Trying to depict the uncertainty of an estimates with a spatial aspect is incredibly difficult. Even displaying an estimates in a spatial context can have problems. Cartograms perform poorly on countries such as Australia which is largely vast open spaces contrasted by dense  coastal cities [@Kobakian]. Often

- uncertainty plots are no different from other plots and should not be treated differently (bivariate maps were originally designed for trying to illustrate two features of a graph)
...
```
- what happens if i swap the "uncertainty" and "estimate"
- can I combine hexmaps with spatial uncertainty maps?
- check what kind of question they can answer
- If you choose two light colours when creating your own palette, the palette will lack important differences in colour across the grid. Therefore, it is best to use two darker colours. 
- While it can express uncertainty, it is very sensistive to colour changes. Makers of the package even note themselves that small changes to the palette and tenciles arguments can generate a completely different map using the same data set. This is ok for data exploration but if we want interesting patterns to jump out at us, they are unlikely to do so.
- one of the main difficulties in data analysis is identifying if a discovery is a feature of the data or a feature of the method we have used. When a method is as sensitive as this
- Part of coming up with these standards is to reduce the amount of work for the statistician. 
- pixilation doesnt work when the area is small.
...
The current research in high dimensional uncertainty visualisation seems to hold with it the implict assumption that the only reason to visualise uncertainty is to validate a structure we see with an estimate. This is not necessarily true.

- I think it if you want to bring an estimate into dispute you need to map the uncertainty to the same feature as the estimate
- if you have done this, the goal is to make sure people take away the correct information from a large structure in the data, they don't just need to be able to answer questions.
...
## Conclusion
Unfortunately visualisation papers that properly consider the underlying distributional information depicted in a plot are rare, and papers that don't conflate issues created by an incorrect distribution, incorrect features, or poor use of aesthetics even more so. One or more of the three issues are frequently attributed to a failing in the plot type and unpacking the source will allow us to make more concentrated efforts towards uncertainty visualisation improvements.
