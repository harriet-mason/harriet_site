...
#| label: fig-charts
#| layout-ncol: 2
#| fig-cap: A table showing the connection between several plots that are commonly considered to be unique but are acutally a spin off of the product plot framework.
#| fig-subcap: ["Charts with no violations to the assumptions", "Charts with violations to the assumptions"]
...
#| fig-cap: "A replication of Figure 12 from the paper. The first row of figures depicts a common product plots while the two plots beneath show that same plot with polar co-ordinates. The second and third row only differ in the mapping of x and y to r and θ. Common plots that are usually not associated with a bar chart, such as a pie chart, can be seen among these figures."
